...
class TestUiPagesSlice4(unittest.TestCase):
    ...
    def test_pages_and_validation(self) -> None:
        with temp_env() as (_, _):
            ...
            self.assertIn('id="global-gdrive-token-btn"', r.text)
            ...
            self.assertIn('id="analyzer-date-period-panel"', analyzer_r.text)
            self.assertIn('id="analyzer-date-from"', analyzer_r.text)
            self.assertIn('id="analyzer-date-to"', analyzer_r.text)
            self.assertIn('id="analyzer-period-compare"', analyzer_r.text)
            self.assertIn('id="analyzer-date-period-apply"', analyzer_r.text)
            self.assertIn('id="analyzer-date-period-clear"', analyzer_r.text)
            self.assertIn('value="PREVIOUS_PERIOD"', analyzer_r.text)
            self.assertIn('value="PREVIOUS_YEAR"', analyzer_r.text)
            self.assertIn('value="NONE"', analyzer_r.text)
            ...
            self.assertIn("buildSurfaceUrlWithDatePeriodFilters", analyzer_r.text)
            self.assertIn("persistDatePeriodQueryParams", analyzer_r.text)
