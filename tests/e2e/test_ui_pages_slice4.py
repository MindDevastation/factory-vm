...
class TestUiPagesSlice4(unittest.TestCase):
    ...
    def test_pages_and_validation(self) -> None:
        with temp_env() as (_, _):
            ...
            self.assertIn("overwrite confirmation required", planner_r.text.lower())
            self.assertIn("selected_items", planner_r.text)
            self.assertIn("selected_fields", planner_r.text)
            self.assertIn("overwrite_confirmed", planner_r.text)
