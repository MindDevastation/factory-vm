...
class TestCustomTagsSeedIo(unittest.TestCase):
    def test_import_missing_file_raises_typed_error(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with temp_env() as (_, _env0):
                ...
                try:
                    with self.assertRaises(catalog_service.SeedValidationError) as ctx:
                        catalog_service.import_catalog(conn, str(seed_dir))
                finally:
                    conn.close()
            self.assertEqual(ctx.exception.code, "CTA_SEED_VALIDATION_FAILED")
    ...
    def test_export_writes_full_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(visual["tags"][0]["code"], "solar")
