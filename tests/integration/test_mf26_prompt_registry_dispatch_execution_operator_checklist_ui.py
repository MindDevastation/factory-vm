...
class TestMf26PromptRegistryDispatchExecutionOperatorChecklistUi(unittest.TestCase):
    ...
    def test_dispatch_execution_operator_checklist_section(self) -> None:
        with temp_env() as (_td, env):
            seed_minimal_db(env)
            client = self._client(env)
            headers = basic_auth_header(env.basic_user, env.basic_pass)
            rec = client.post("/v1/prompt-registry/records", headers=headers, json={"slug": "mf26", "code": "MF26", "title": "mf26", "record_type": "prompt_template", "status": "draft"})
            prompt_id = int(rec.json()["id"])
            action = client.post(f"/v1/prompt-registry/records/{prompt_id}/linked-actions", headers=headers, json={"action_key": "mf26-a", "action_type": "ui_action", "action_status": "active", "target_kind": "route", "target_ref": "/ui/prompt-registry", "config_json": {}})
            ...
            self.assertIn("Execution operator checklist", detail.text)
            self.assertIn("Operator checklist only. No runtime execution is performed.", detail.text)
            self.assertIn("checklist_status", detail.text)
            self.assertIn("BLOCKED", detail.text)
            self.assertIn("NO_RUNTIME_ACTION_AVAILABLE", detail.text)
            self.assertIn("review_preflight", detail.text)
