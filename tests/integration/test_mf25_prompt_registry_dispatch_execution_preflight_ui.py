...
class TestMf25PromptRegistryDispatchExecutionPreflightUi(unittest.TestCase):
    ...
    def test_dispatch_execution_preflight_summary_section(self) -> None:
        with temp_env() as (_td, env):
            seed_minimal_db(env)
            client = self._client(env)
            headers = basic_auth_header(env.basic_user, env.basic_pass)
            rec = client.post("/v1/prompt-registry/records", headers=headers, json={"slug": "mf25", "code": "MF25", "title": "mf25", "record_type": "prompt_template", "status": "draft"})
            prompt_id = int(rec.json()["id"])
            action = client.post(f"/v1/prompt-registry/records/{prompt_id}/linked-actions", headers=headers, json={"action_key": "mf25-a", "action_type": "ui_action", "action_status": "active", "target_kind": "route", "target_ref": "/ui/prompt-registry", "config_json": {}})
            ...
            self.assertIn("Preflight summary only. No runtime execution is performed.", detail.text)
            self.assertIn("preflight_status", detail.text)
            self.assertIn("execution_enabled", detail.text)
            self.assertIn("DISPATCH_EXECUTION_NOT_IMPLEMENTED", detail.text)
