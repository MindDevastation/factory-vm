...
class TestMf27PromptRegistryDispatchExecutionOperatorHandoffUi(unittest.TestCase):
    ...
    def test_dispatch_execution_operator_handoff_section(self) -> None:
        with temp_env() as (_td, env):
            ...
            self.assertIn("blocking_codes", detail.text)
            self.assertIn("warning_codes", detail.text)
            self.assertIn("NO_RUNTIME_ACTION_AVAILABLE", detail.text)
            self.assertIn("Execution operator handoff checklist items", detail.text)
            self.assertIn("PREFLIGHT_LOADED", detail.text)
            self.assertIn("Execution operator handoff audit payload preview", detail.text)
