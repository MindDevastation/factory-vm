...
class TestMf23PromptRegistryDispatchExecutionCapabilityUi(unittest.TestCase):
    ...
    def test_dispatch_execution_capability_section_is_read_only(self) -> None:
        with temp_env() as (_td, env):
            seed_minimal_db(env)
            client = self._client(env)
            headers = basic_auth_header(env.basic_user, env.basic_pass)
            rec = client.post("/v1/prompt-registry/records", headers=headers, json={"slug": "mf23", "code": "MF23", "title": "mf23", "record_type": "prompt_template", "status": "draft"})
            prompt_id = int(rec.json()["id"])
            action = client.post(f"/v1/prompt-registry/records/{prompt_id}/linked-actions", headers=headers, json={"action_key": "mf23-a", "action_type": "ui_action", "action_status": "active", "target_kind": "route", "target_ref": "/ui/prompt-registry", "config_json": {}})
            ...
            self.assertIn("Capability matrix only. No runtime execution is performed.", detail.text)
            self.assertIn("capability_status", detail.text)
            self.assertIn("support_level", detail.text)
