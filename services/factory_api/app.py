...
def _oauth_start(kind: str, channel_slug: str) -> dict:
    ...
    nonce = _new_oauth_session_nonce()
    state = sign_state(secret=env.oauth_state_secret, kind=kind, channel_slug=channel_slug, nonce=nonce)
    ...
    _write_oauth_code_verifier(nonce, auth.code_verifier)
...
def _oauth_global_gdrive_start() -> dict[str, str]:
    ...
    nonce = _new_oauth_session_nonce()
    state = sign_state(secret=env.oauth_state_secret, kind="gdrive_global", nonce=nonce)
    auth = build_authorization_url(
        client_secret_path=client_secret_path,
        scope=scope,
...
def _oauth_global_gdrive_callback(code: str, state: str) -> HTMLResponse:
    ...
    code_verifier = _read_oauth_code_verifier(nonce)
    token_json = _exchange_code_for_token_json_safe(
        client_secret_path=client_secret_path,
        scope=scope,
        redirect_uri=redirect_uri(env, "gdrive_global"),
        code=code,
    ...
    _delete_oauth_code_verifier(nonce)
    token_path = _global_gdrive_token_path(env)
    ensure_token_dir(token_path)
...
def _new_oauth_session_nonce() -> str:
    return secrets.token_urlsafe(18)


def _oauth_state_nonce(payload: dict[str, Any]) -> str:
    nonce = str(payload.get("nonce") or "").strip()
    if not nonce or not re.fullmatch(r"[A-Za-z0-9_-]+", nonce):
        raise HTTPException(400, "invalid oauth state")
    return nonce


def _oauth_code_verifier_path(nonce: str) -> Path:
    return _storage_tmp_oauth_dir() / f"{nonce}.verifier"


def _write_oauth_code_verifier(nonce: str, code_verifier: str) -> Path:
    tmp_dir = _storage_tmp_oauth_dir()
    tmp_dir.mkdir(parents=True, mode=0o700, exist_ok=True)
    verifier_path = _oauth_code_verifier_path(nonce)
    verifier_path.write_text(code_verifier, encoding="utf-8")
    verifier_path.chmod(0o600)
    return verifier_path


def _read_oauth_code_verifier(nonce: str) -> str:
    verifier_path = _oauth_code_verifier_path(nonce)
...
def _delete_oauth_code_verifier(nonce: str) -> None:
    verifier_path = _oauth_code_verifier_path(nonce)
    if verifier_path.is_file():
        verifier_path.unlink()


def _exchange_code_for_token_json_safe(**kwargs: Any) -> str:
    try:
        return exchange_code_for_token_json(**kwargs)
    except HTTPException:
        raise
    except Exception as exc:
        detail = str(exc).lower()
        exc_name = exc.__class__.__name__.lower()
        if "missing code verifier" in detail or ("invalidgrant" in exc_name and "code verifier" in detail):
            raise HTTPException(400, "OAuth token exchange failed: missing or expired authorization session. Start OAuth again.") from exc
        raise
...
def api_oauth_youtube_add_channel_start(_: bool = Depends(require_basic_auth(env))):
    ...
    nonce = _new_oauth_session_nonce()
    state = sign_state(secret=env.oauth_state_secret, kind="youtube_add_channel", nonce=nonce)
    auth = build_authorization_url(
        client_secret_path=client_secret_path,
        scope=scope,
...
def _ui_prompt_registry_dispatch_attempt_detail_context(
    ...
    *,
    ...
    execution_audit_preview = item.get("execution_audit_preview") if isinstance(item.get("execution_audit_preview"), dict) else {}
    preview_payload = execution_audit_preview.get("audit_payload_preview") if isinstance(execution_audit_preview.get("audit_payload_preview"), dict) else {}
    item["execution_audit_preview"] = {
        "preview_status": execution_audit_preview.get("preview_status"),
        "would_write_event_type": execution_audit_preview.get("would_write_event_type"),
        "would_write": bool(execution_audit_preview.get("would_write")),
        "reason_code": execution_audit_preview.get("reason_code"),
        "execution_status": execution_audit_preview.get("execution_status"),
        "readiness_status": execution_audit_preview.get("readiness_status"),
        "capability_status": execution_audit_preview.get("capability_status"),
        "runtime_available": bool(execution_audit_preview.get("runtime_available")),
        "blockers": execution_audit_preview.get("blockers") if isinstance(execution_audit_preview.get("blockers"), list) else [],
        "warnings": execution_audit_preview.get("warnings") if isinstance(execution_audit_preview.get("warnings"), list) else [],
        "notes": execution_audit_preview.get("notes") if isinstance(execution_audit_preview.get("notes"), list) else [],
        "audit_payload_preview": {
            "attempt_id": preview_payload.get("attempt_id"),
            "execution_request_id": preview_payload.get("execution_request_id"),
            "prompt_id": preview_payload.get("prompt_id"),
            "action_id": preview_payload.get("action_id"),
            "dispatch_status": preview_payload.get("dispatch_status"),
            "dispatch_kind": preview_payload.get("dispatch_kind"),
            "dispatch_target": preview_payload.get("dispatch_target"),
            "readiness_status": preview_payload.get("readiness_status"),
            "capability_status": preview_payload.get("capability_status"),
            "reason_code": preview_payload.get("reason_code"),
