...
def create_publish_bulk_actions_router(env: Env) -> APIRouter:
    ...
    def preview(payload: PublishBulkPreviewPayload, request: Request, _: bool = Depends(require_basic_auth(env))):
        ...
        try:
            return create_bulk_preview_session(
                ...
                scheduled_at=(str(payload.scheduled_at or "").strip() or None),
    ...
    def execute(payload: PublishBulkExecutePayload, request: Request, _: bool = Depends(require_basic_auth(env))):
        conn = dbm.connect(env)
        try:
            return execute_bulk_preview_session(
                conn,
...
def create_bulk_preview_session(
    ...
    *,
    ...
    action_payload = _normalize_action_payload(action=normalized_action, scheduled_at=scheduled_at)
    ...
    for job_id in normalized_ids:
        ...
        if job is None:
            ...
            continue
        preview_item = _preview_item(action=normalized_action, action_payload=action_payload, job=dict(job))
    ...
    fingerprint = _build_selection_fingerprint(action=normalized_action, selected_job_ids=snapshot_ids, action_payload=action_payload)
    aggregate = {
    ...
    dbm.insert_publish_bulk_action_session(
        ...
        action_payload_json=dbm.json_dumps(action_payload),
...
def execute_bulk_preview_session(
    ...
    *,
    ...
    action_payload = dbm.json_loads(str(session.get("action_payload_json") or "{}"))
    ...
    if len(set(execute_ids)) != len(execute_ids):
        request_fingerprint = _build_selection_fingerprint(action=action, selected_job_ids=execute_ids, action_payload=action_payload)
    ...
    if any(job_id not in set(snapshot_ids) for job_id in execute_ids):
        request_fingerprint = _build_selection_fingerprint(action=action, selected_job_ids=execute_ids, action_payload=action_payload)
        raise PublishBulkActionError(
            code="PBA_EXECUTE_OUTSIDE_SNAPSHOT",
    ...
    request_fingerprint = selection_fingerprint or _build_selection_fingerprint(
        action=action,
        selected_job_ids=execute_ids,
        action_payload=action_payload,
    )
        ...
        for job_id in execute_ids:
            ...
            items.append(
                _execute_item(
                    ...
                    action_payload=action_payload,
                    job_id=job_id,
                    job_row=(dict(row) if row else None),
                )
            )
...
def _normalize_action_payload(*, action: str, scheduled_at: str | None) -> dict[str, Any]:
    if action != "reschedule":
        return {}

    parsed = _parse_iso_datetime(str(scheduled_at or ""), request_id="bulk-reschedule", field_name="scheduled_at")
    if isinstance(parsed, JSONResponse):
        raise PublishBulkActionError(code="PJA_INVALID_DATETIME", message="scheduled_at must be ISO datetime", status_code=422)
    if float(parsed) <= dbm.now_ts():
        raise PublishBulkActionError(code="PJA_RESCHEDULE_NOT_FUTURE", message="scheduled_at must be in the future", status_code=422)

    normalized_iso = datetime.fromtimestamp(float(parsed), tz=timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    return {"scheduled_at": normalized_iso, "scheduled_at_ts": float(parsed)}
...
def _preview_item(*, action: str, action_payload: dict[str, Any], job: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "reschedule":
        item["scheduled_at"] = action_payload["scheduled_at"]
    return item
    ...
    *,
    ...
    if action == "hold":
        ...
        conn.execute(
            ...
            ("operator_forced_manual", now_ts, now_ts, job_id),
        )
        return {
            "job_id": job_id,
            "action": action,
    ...
    mutation = _run_transition_action(conn, action=action, action_payload=action_payload, row=job_row)
    ...
    if action == "reschedule":
        out["scheduled_at"] = action_payload["scheduled_at"]
    return out
    ...
    *,
    ...
    if action == "reschedule":
        return _apply_publish_transition(
            ...
            updates={"publish_scheduled_at": float(action_payload["scheduled_at_ts"]), "publish_retry_at": None},
        )
    if action == "unblock":
        return _apply_publish_transition(
            conn,
...
def _build_selection_fingerprint(*, action: str, selected_job_ids: list[int], action_payload: dict[str, Any]) -> str:
    payload = {
        "action": action,
        "selected_job_ids": [int(item) for item in selected_job_ids],
        "action_payload": action_payload,
    }
