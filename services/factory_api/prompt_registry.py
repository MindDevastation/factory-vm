...
def create_prompt_registry_router(env: Env) -> APIRouter:
    ...
    @router.get("/linked-action-dispatch-attempts/{attempt_id}/execution-capability")
    def get_linked_action_dispatch_execution_capability(attempt_id: int, _: bool = Depends(require_basic_auth(env))):
        conn = dbm.connect(env)
        try:
            return PromptRegistryService(conn).evaluate_linked_action_dispatch_execution_capability(attempt_id)
        except PromptRegistryNotFoundError as exc:
            return _error("PROMPT_REGISTRY_NOT_FOUND", str(exc), status_code=404)
        except (TypeError, ValueError, PromptRegistryValidationError) as exc:
            return _error("PROMPT_REGISTRY_VALIDATION_ERROR", str(exc), status_code=422)
    ...
    @router.get("/linked-action-dispatch-attempts/{attempt_id}/execution-operator-checklist")
    def get_linked_action_dispatch_execution_operator_checklist(attempt_id: int, _: bool = Depends(require_basic_auth(env))):
        conn = dbm.connect(env)
        try:
            return PromptRegistryService(conn).preview_linked_action_dispatch_execution_operator_checklist(attempt_id)
        except PromptRegistryNotFoundError as exc:
            return _error("PROMPT_REGISTRY_NOT_FOUND", str(exc), status_code=404)
        except (TypeError, ValueError, PromptRegistryValidationError) as exc:
            return _error("PROMPT_REGISTRY_VALIDATION_ERROR", str(exc), status_code=422)
    ...
    @router.get("/linked-action-dispatch-attempts/{attempt_id}/execution-operator-handoff")
    def get_linked_action_dispatch_execution_operator_handoff(attempt_id: int, _: bool = Depends(require_basic_auth(env))):
        conn = dbm.connect(env)
        try:
            return PromptRegistryService(conn).preview_linked_action_dispatch_execution_operator_handoff(attempt_id)
        except PromptRegistryNotFoundError as exc:
            return _error("PROMPT_REGISTRY_NOT_FOUND", str(exc), status_code=404)
        except (TypeError, ValueError, PromptRegistryValidationError) as exc:
            return _error("PROMPT_REGISTRY_VALIDATION_ERROR", str(exc), status_code=422)
    ...
    def get_linked_action_dispatch_execution_audit_preview(attempt_id: int, _: bool = Depends(require_basic_auth(env))):
        conn = dbm.connect(env)
        try:
            return PromptRegistryService(conn).preview_linked_action_dispatch_execution_audit(attempt_id)
        except PromptRegistryNotFoundError as exc:
            return _error("PROMPT_REGISTRY_NOT_FOUND", str(exc), status_code=404)
        except (TypeError, ValueError, PromptRegistryValidationError) as exc:
            return _error("PROMPT_REGISTRY_VALIDATION_ERROR", str(exc), status_code=422)
