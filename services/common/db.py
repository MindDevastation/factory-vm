...
def migrate(conn: sqlite3.Connection) -> None:
    ...
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS analytics_prediction_snapshots (
            ...
            updated_at REAL NOT NULL,
            CHECK(scope_type IN ('CHANNEL', 'RELEASE', 'BATCH_MONTH', 'PORTFOLIO')),
            CHECK(prediction_family IN ('VIEW_GROWTH_PREDICTION', 'WATCH_TIME_GROWTH_PREDICTION', 'CTR_PREDICTION', 'STRONG_WEAK_RELEASE_PREDICTION', 'BEST_PUBLISH_WINDOW_PREDICTION', 'CHANNEL_TREND_PREDICTION', 'ANOMALY_DROP_RISK_PREDICTION')),
            CHECK(variance_class IN ('NORMAL', 'ANOMALY', 'RISK')),
        ...
        CREATE TABLE IF NOT EXISTS analytics_prediction_events (
            ...
            CHECK(target_scope_type IN ('CHANNEL', 'RELEASE', 'BATCH_MONTH', 'PORTFOLIO')),
            CHECK(run_kind IN ('BASELINE_RECOMPUTE', 'COMPARISON_RECOMPUTE', 'PREDICTION_RECOMPUTE', 'FULL_STACK_RECOMPUTE')),
            CHECK(prediction_family IS NULL OR prediction_family IN ('VIEW_GROWTH_PREDICTION', 'WATCH_TIME_GROWTH_PREDICTION', 'CTR_PREDICTION', 'STRONG_WEAK_RELEASE_PREDICTION', 'BEST_PUBLISH_WINDOW_PREDICTION', 'CHANNEL_TREND_PREDICTION', 'ANOMALY_DROP_RISK_PREDICTION')),
            CHECK(comparison_family IS NULL OR comparison_family IN ('RELEASE_VS_CHANNEL_BASELINE', 'CHANNEL_VS_SELF_HISTORY', 'BATCH_MONTH_VS_RECENT_CHANNEL', 'CHANNEL_VS_PORTFOLIO')),
        ...
        CREATE TABLE IF NOT EXISTS analytics_recommendation_runs (
            ...
            CHECK(recommendation_family IN ('PUBLISH_TIMING_SUGGESTION', 'CADENCE_BATCH_HEALTH_SUGGESTION', 'WEAK_RELEASE_ATTENTION', 'TITLE_METADATA_IMPROVEMENT', 'VISUAL_IMPROVEMENT', 'OPERATIONAL_REMEDIATION', 'CHANNEL_OPTIMIZATION', 'ANOMALY_RISK_ALERT', 'CONTENT_PACKAGING_SUGGESTION', 'CONTENT_PLANNING_SUGGESTION')),
            CHECK(recompute_mode IN ('FULL_RECOMPUTE', 'INCREMENTAL_RECOMPUTE', 'TARGETED_RECOMPUTE')),
            CHECK(run_state IN ('RUNNING', 'SUCCEEDED', 'PARTIAL', 'FAILED'))
        );
        ...
        CREATE TABLE IF NOT EXISTS analytics_recommendation_events (
            ...
            created_at REAL NOT NULL,
            CHECK(recommendation_scope_type IN ('CHANNEL', 'RELEASE', 'BATCH_MONTH', 'PORTFOLIO')),
            CHECK(recommendation_family IS NULL OR recommendation_family IN ('PUBLISH_TIMING_SUGGESTION', 'CADENCE_BATCH_HEALTH_SUGGESTION', 'WEAK_RELEASE_ATTENTION', 'TITLE_METADATA_IMPROVEMENT', 'VISUAL_IMPROVEMENT', 'OPERATIONAL_REMEDIATION', 'CHANNEL_OPTIMIZATION', 'ANOMALY_RISK_ALERT', 'CONTENT_PACKAGING_SUGGESTION', 'CONTENT_PLANNING_SUGGESTION')),
...
def list_channel_playlists(conn: sqlite3.Connection, channel_id: int) -> List[Dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT playlist_id, playlist_title
        FROM channel_playlists
        WHERE channel_id = ? AND is_active = 1
        ORDER BY playlist_title ASC, playlist_id ASC
        """,
        (channel_id,),
    ).fetchall()
